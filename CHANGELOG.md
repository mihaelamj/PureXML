...
- HTML named character references: the full HTML 4.01 entity set (#84), about 250 references across the Latin-1, special, and symbol (Greek, mathematical, arrow, card-suit) groups, replacing the previous 16-entry stopgap. Decoding stays longest-match with an optional trailing semicolon and the existing numeric-reference validation. This matches libxml2's entity coverage; the much larger WHATWG superset of mostly-astral mathematical references is not vendored.
