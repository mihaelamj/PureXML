...
- Shift-JIS decoding (#99, stage 1): the byte decoder gains a multi-byte path and the WHATWG Shift-JIS algorithm, decoding ASCII, half-width katakana, and the JIS X 0208 two-byte plane (its index vendored verbatim from the WHATWG `index-jis0208.txt`), for both the whole-buffer and streaming decoders, selected by the XML declaration's `encoding` name (`Shift_JIS`, `sjis`, `windows-31j`, ...). The remaining CJK encodings (EUC-JP/KR, GB2312/GBK) are tracked under the same issue.
