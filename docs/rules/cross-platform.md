...
How to structure PureXML's Swift so the same sources build and run on Apple
...
Grounded in Swift Evolution and `swiftlang/swift-foundation`. The PureXML
...
3. **Cross-platform library or CLI.** Runs identically on Apple + Linux +
   ...
   Darwin/Glibc/WinSDK conditionals, swift-system. The PureXML engine lives
...
let appleOnlyProducts: [Product] = [
    .singleTargetLibrary("PureXMLUI"),
    .singleTargetLibrary("PureXMLComponents"),
    // ...
]
#else
let appleOnlyProducts: [Product] = []
...
Same pattern for `appleOnlyTargets`. Core engine products (e.g. `PureXML`) stay unconditional.
...
```swift
#if os(iOS)
return "PureXML/iOS/\(version)"
#elseif os(macOS)
return "PureXML/macOS/\(version)"
#elseif os(visionOS)
return "PureXML/visionOS/\(version)"
#else
return "PureXML/unknown/\(version)"  // Linux falls into "unknown"
#endif
```
...
return "PureXML/visionOS/\(version)"
#elseif os(Linux)
return "PureXML/Linux/\(version)"
#else
return "PureXML/unknown/\(version)"  // truly unknown future platform
#endif
```
