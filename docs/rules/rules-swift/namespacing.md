...
Single-page rule for picking the namespace-anchor type in TileKit and the wider Tiledown codebase: when the anchor is `Foo`, should it be `enum Foo`, `struct Foo`, or `class Foo`?
