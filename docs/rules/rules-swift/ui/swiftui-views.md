...
How a Tiledown-style native editor writes its SwiftUI renderer: purely presentational, performant, accessible, reusable. Views observe view models but contain ZERO business logic. This is one of three Apple renderers over the same framework-free model (see `pre-ui-layer.md`); its siblings are `uikit-views.md` and `appkit-views.md`.
