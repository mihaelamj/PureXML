...
How to register custom fonts in a Tiledown-style native editor: CoreText registration inside SPM packages, never the Info.plist approaches. Resources must use `.process()` in Package.swift, and resource access always goes through `Bundle.module`.
