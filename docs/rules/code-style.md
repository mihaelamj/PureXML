...
The operational namespacing discipline for PureXML: folder mirroring, file naming, anchor placement, one-type-per-file, reserved names, namespace-vs-module collisions.
...
This applies to every Swift project. The PureXML-style root + sub-namespace pattern below is one specific instance of this discipline.
...
```text
PureXML.Parser.Model.Token          // from PureXMLCore
PureXML.Generator.Command           // from PureXMLCLI
PureXML.Parser.Scanning.Scanner     // from PureXMLTools
```
...
## Root + sub-namespace pattern (PureXML style)
...
```swift
/// Root namespace for all PureXML types across all modules
public enum PureXML {
...
- Root namespace = one file (e.g. `PureXML.swift`)
...
extension PureXML.Parser.Model {
...
extension PureXML.Parser.Model.Token: CustomStringConvertible {
...
- Type lives under the correct semantic path, e.g.:
  - `PureXML.Model`
  - `PureXML.Parser.Model`
  - `PureXML.Parser.Metadata`
  - `PureXML.Generator`
- Conformances (`Codable`, `Sendable`, `CustomStringConvertible`, etc.) MAY be separate extensions

### File layout for PureXML-style namespacing
...
```text
PureXML/
    PureXML.swift                       // namespace anchor: public enum PureXML {}
    Parser/Model/Token.swift             // extension PureXML.Parser.Model { public enum Token {} }
    Parser/Scanning/Scanner.swift        // extension PureXML.Parser.Scanning { public struct Scanner {} }
    Generator/Writer.swift               // extension PureXML.Generator { public struct Writer {} }
```
...
```text
PureXML.swift                              // namespace anchor: public enum PureXML {}
PureXML.Parser.Model.Token.swift           // extension PureXML.Parser.Model { public enum Token {} }
PureXML.Parser.Metadata.Header.swift       // extension PureXML.Parser.Metadata { public struct Header {} }
```
...
- `PureXML.swift` declaring `public enum PureXML { /* sub-namespaces */ }`
...
Concrete example, a `PureXML` namespace umbrella:

```text
Sources/PureXML/
├── PureXML.swift          // namespace anchor: extension PureXML { ... }      <-- AT ROOT, not Core/PureXML.swift
├── Client/                // PureXMLClient SPM target
├── Core/                  // PureXMLCore SPM target (path includes ../PureXML.swift)
│   ├── Protocol/
│   ├── Server/
│   └── Transport/
├── SharedTools/           // PureXMLSharedTools SPM target
└── Support/               // PureXMLSupport SPM target
```

The PureXMLCore target's `Package.swift` entry uses `path: "Sources/PureXML"` with `exclude: ["Client", "SharedTools", "Support"]` so it picks up the anchor file plus the `Core/` subtree. Every sibling target (Client, SharedTools, Support) depends on PureXMLCore in production, so the `PureXML` namespace anchor reaches them through that dep.

Why this matters:

- Reading the file listing immediately tells you "PureXML is the umbrella here": the anchor sits visually at the top.
- Moving an anchor into `Core/PureXML.swift` (or any other sub-target folder) hides the umbrella relationship behind one extra `cd` and one extra mental indirection.
