...
Canonical dependency-injection principles for PureXML and the wider PureXML codebase: no singletons, constructor injection, protocol seams.
...
If and when PureXML grows into multiple SPM packages:
...
If and when PureXML is split into multiple packages, each package should lift out of the repo without modification:
...
If PureXML ships multiple binaries, the **executable target name** stays normal (no `Impl` suffix). The binary's external identity (filename on disk, `which` output) stays clean.
