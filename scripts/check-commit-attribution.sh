#!/usr/bin/env bash
# Scan commit MESSAGES (and author/committer identity) in a range for
# AI-attribution tells. Tool-agnostic.
#
...
# AI-agent author/committer identity. Conservative on names (a human may be
# named Claude or Gemini); matches agent email domains and the Cursor agent
# identity, which injects itself in practice.
IDENT_REGEX='@cursor\.com|cursoragent|(^|[^[:alpha:]])[Cc]ursor [Aa]gent([^[:alpha:]]|$)'
...
for sha in $COMMITS; do
  ...
  if printf '%s' "$IDENT" | grep -qiE -- "$IDENT_REGEX"; then
    printf 'commit-attribution: %s has an AI-agent author/committer identity:\n' "$short" >&2
    printf '%s\n' "$IDENT" | grep -niE -- "$IDENT_REGEX" | sed 's/^/    /' >&2
    FAIL=1
  fi
done
