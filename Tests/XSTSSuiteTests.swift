...
struct XSTSSuiteTests {
    ...
    /// Instance bucket (133 -> 60), per-step deltas in CHANGELOG.md: xsi:type must derive from the
    ...
    /// identity fields compare in the decimal value space; a complex type's attribute wildcard is the
    /// INTERSECTION of its `anyAttribute` sources.
    private let knownInstanceInvalidAccepted = 60
