// The recursion guard must be exercised on a large stack (the limit is above
// what a small stack holds), which needs Foundation's Thread. WASI is
// single-threaded and has no Thread/Dispatch, so these tests are excluded
// there; the guard itself is platform-independent pure Swift, covered by the
