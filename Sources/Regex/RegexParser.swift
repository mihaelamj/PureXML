    ...
    struct RegexParser {
        ...
        private mutating func parseCategory(negated: Bool) throws -> CategoryPredicate {
            ...
            guard let predicate = CategoryPredicate(name: name, negated: negated) else {
                // The recognised set (every Unicode general category plus the
                // complete XSD 1.0 `Is<Block>` list, see CategoryMatcher) is the
                // exhaustive set of XSD `charProp` names, so any name outside it is
                // no XSD category or block and is a genuine syntax error, not an
                // engine limitation. Rejecting it (XSTS reK88, `\p{IsaA0-a9}`)
                // cannot reject a valid pattern because the set is complete.
                throw RegexError.invalidProperty
            }
            return predicate
        }
