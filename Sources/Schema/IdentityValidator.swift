extension PureXML.Schema {
    ...
    struct IdentityValidator {
        ...
        /// targets, keyed the same way as ``fieldTypes``. When a field selects an
        /// absent attribute or an empty element whose declaration carries one of
        /// these, the field's identity value is that value, not nothing.
        let fieldConstraints: [String: PureXML.Schema.ValueConstraint]
        ...
        init(
            ...
            fieldConstraints: [String: PureXML.Schema.ValueConstraint] = [:],
        ...
        private func fieldTuple(_ fields: [String], at node: PureXML.Model.TreeNode, constraint: IdentityConstraint) -> [FieldValue?] {
            ...
            return fields.map { field in
                ...
                // An absent attribute (empty node-set) or an empty element whose
                // declaration carries a `default`/`fixed` takes that value as its
                // identity component. A present attribute (even value "") and a
                // non-empty element are instance-supplied and never defaulted.
                if let defaulted = defaultedValue(field, value: value, type: type, constraint: constraint, at: node) {
                    return defaulted
                }
                if let nodes = value.nodes, nodes.isEmpty { return nil }
                return FieldValue(string: value.string, type: type ?? Self.effectiveType(of: value), namespaceBindings: instanceNamespaces)
            }
        }

        /// Prefix bindings in scope at `node`, gathered from ancestor elements.
        func namespaceBindings(at node: PureXML.Model.TreeNode) -> [String: String] {
